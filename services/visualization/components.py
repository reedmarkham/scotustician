...
def render_cluster_representatives(
    ...
    # Default to hdbscan if available
    cluster_methods = [col for col in df.columns if col.endswith('_cluster')]
    selected_method = 'hdbscan_cluster' if 'hdbscan_cluster' in cluster_methods else cluster_methods[0]
            ...
            for i, term in enumerate(selected_terms):
                with cols[i % len(cols)]:
                    if term in term_data:
                        ...
                        metadata = term_data[term].get('metadata')
                        cluster_reps = metadata.get('cluster_representatives') if metadata else None
                        fig = visualizer.create_cluster_scatter_plot(df, 'hdbscan_cluster', cluster_reps)
